...
class SnappingPosition {
  ...
  double getPositionInPixels(double maxHeight, double grabbingHeight) {
    var centerPosition = _getCenterPositionInPixels(maxHeight);
    var centerOffset = grabbingHeight * grabbingContentOffset / 2;
    return centerPosition + centerOffset;
  }

  double? get pixel => _positionPixel;

  double _getCenterPositionInPixels(double maxHeight) {
    if (_positionPixel != null) return _positionPixel!;
    return _positionFactor! * maxHeight;
  }

  @override
  bool operator ==(other) =>
      other is SnappingPosition &&
      other._positionFactor == _positionFactor &&
      other._positionPixel == _positionPixel;
